...
public class StripePaymentProvider implements PaymentProvider {
    ...
    public PaymentSession createPayment(CreatePaymentCommand c) {
        ...
        form.add("success_url", c.returnUrl());
        form.add("cancel_url", c.returnUrl());
        form.add("line_items[0][quantity]", "1");
        form.add("line_items[0][price_data][currency]", c.currency().name().toLowerCase());
        form.add(
            "line_items[0][price_data][unit_amount]",
            c.amount().movePointRight(c.currency().minorUnits()).toBigInteger().toString()
        );
        ...
        if (r == null || r.id() == null || r.url() == null) {
            throw new IllegalStateException("Stripe returned an invalid checkout response");
        }
    ...
    public void cancelPayment(String id) {
        throw new UnsupportedOperationException("Stripe Checkout cancellation requires session-specific integration");
    }

    @Override
    public void refundPayment(String id, BigDecimal amount, Currency currency) {
        throw new UnsupportedOperationException("Stripe refunds require payment transaction integration");
    }

    @Override
    public WebhookResult handleWebhook(String payload, Map<String, String> headers) {
        requireConfigured();
        verifySignature(payload, headers.get("Stripe-Signature"));
        ...
        try {
            ...
            if (id.isBlank()) {
                throw new IllegalArgumentException("Stripe webhook has no checkout session id");
            }

            String reference = object.path("metadata").path("reference").asText(null);
            String currencyCode = object.path("currency").asText(null);
            Currency currency = currencyCode == null || currencyCode.isBlank()
                ? null
                : Currency.valueOf(currencyCode.toUpperCase());

            BigDecimal amount = object.path("amount_total").isNumber()
                ? object.path("amount_total").decimalValue().movePointLeft(currency == null ? 2 : currency.minorUnits())
                : null;
    ...
    private PaymentStatus map(String status) {
    ...
    private PaymentStatus map(String type, String status) {
    ...
    private void verifySignature(String payload, String signature) {
        ...
        if (signature == null) {
            throw new SecurityException("Missing Stripe webhook signature");
        }
        ...
        try {
            ...
            for (String p : parts) {
                ...
                if ("v1".equals(kv[0])) values.add(kv[1]);
            }
            ...
            if (Math.abs(System.currentTimeMillis() / 1000 - ts) > 300) {
                throw new SecurityException("Expired Stripe webhook signature");
            }
            ...
            throw new SecurityException("Unable to verify Stripe webhook signature", e);
        }
    }
